...
//
// ВАЖНО: числа отсюда — это значения ПО УМОЛЧАНИЮ. Рабочие значения администратор
// правит в разделе «Настройки → Данные для расчёта» (хранятся в БД); расчёт берёт
// их из `CalcConfig` — см. `lib/calc-config.ts` и `lib/settings.ts`. Списки опций
// (REGION_OPTIONS и т.п.), названия и порядок направлений через интерфейс не меняются.
